...
function buildOnePlanner() {
  ...
  // 選択された要素を追加
  p.push(...[
    getOne("pl_bg"), getOne("pl_pose"), getOne("pl_comp"), 
    getOne("pl_view"), getOne("pl_expr"), getOne("pl_light")
  ].filter(Boolean));
  ...
  if (importChar) {
    importChar.addEventListener("change", async (e) => {
      ...
      try {
        ...
        // 服のモードを復元
        if (data.outfitMode) {
          const radio = document.querySelector(`input[name="outfitMode"][value="${data.outfitMode}"]`);
          if (radio) {
            radio.checked = true;
            window.applyOutfitMode?.(); // モード切り替えを反映
          }
        }
        ...
        if (data.hairColor) {
          const satH = document.getElementById("satH");
          const litH = document.getElementById("litH");
          if (satH && data.hairColor.s !== undefined) satH.value = data.hairColor.s;
          if (litH && data.hairColor.l !== undefined) litH.value = data.hairColor.l;
          if (data.hairColor.h !== undefined) {
            setTimeout(() => {
              if (window.getHairColorTag && typeof window.getHairColorTag === 'function') {
                window.getHairColorTag.__lastHue = data.hairColor.h;
                paintHairColor?.(data.hairColor.h);
              }
            }, 200);
          }
        }
        ...
        if (data.eyeColor) {
          ...
          if (satE && data.eyeColor.s !== undefined) satE.value = data.eyeColor.s;
          if (litE && data.eyeColor.l !== undefined) litE.value = data.eyeColor.l;
          if (data.eyeColor.h !== undefined && window.getEyeColorTag) {
            setTimeout(() => {
              window.getEyeColorTag.__lastHue = data.eyeColor.h;
            }, 200);
          }
        }
        ...
        // 服の色を復元
        ['top', 'bottom', 'shoes'].forEach(colorType => {
          const colorData = data[`${colorType}Color`];
          if (colorData) {
            const satEl = document.getElementById(`sat_${colorType}`);
            const litEl = document.getElementById(`lit_${colorType}`);
            const useEl = document.getElementById(`use_${colorType}`);
            
            if (satEl && colorData.s !== undefined) satEl.value = colorData.s;
            if (litEl && colorData.l !== undefined) litEl.value = colorData.l;
            if (useEl && colorData.enabled !== undefined) useEl.checked = colorData.enabled;
            
            if (colorData.h !== undefined) {
              setTimeout(() => {
                const colorFunc = window[`get${colorType.charAt(0).toUpperCase()}${colorType.slice(1)}Color`];
                if (colorFunc && colorFunc.__lastHue !== undefined) {
                  colorFunc.__lastHue = colorData.h;
                }
              }, 200);
            }
          }
        ...
        // チェックボックス項目の復元
        if (data.useBottomColor !== undefined) {
          const useBottomColor = document.getElementById("useBottomColor");
          if (useBottomColor) useBottomColor.checked = data.useBottomColor;
        }
  ...
  if (exportChar) {
    exportChar.addEventListener("click", () => {
      const data = {
        ...
        // 色情報（髪・目・肌）
        hairColor: {
          h: window.getHairColorTag?.__lastHue || 35,
          s: parseInt(document.getElementById("satH")?.value || 70),
          l: parseInt(document.getElementById("litH")?.value || 45)
        },
        eyeColor: {
          h: window.getEyeColorTag?.__lastHue || 240,
          s: parseInt(document.getElementById("satE")?.value || 80),
          l: parseInt(document.getElementById("litE")?.value || 55)
        },
        skinTone: parseInt(document.getElementById("skinTone")?.value || 30),
        // 服の色情報
        topColor: {
          h: window.getTopColor?.__lastHue || 35,
          s: parseInt(document.getElementById("sat_top")?.value || 80),
          l: parseInt(document.getElementById("lit_top")?.value || 55),
          enabled: document.getElementById("use_top")?.checked || false
        },
        bottomColor: {
          h: window.getBottomColor?.__lastHue || 210,
          s: parseInt(document.getElementById("sat_bottom")?.value || 70),
          l: parseInt(document.getElementById("lit_bottom")?.value || 50),
          enabled: document.getElementById("useBottomColor")?.checked || false
        },
        shoesColor: {
          h: window.getShoesColor?.__lastHue || 0,
          s: parseInt(document.getElementById("sat_shoes")?.value || 0),
          l: parseInt(document.getElementById("lit_shoes")?.value || 30),
          enabled: document.getElementById("use_shoes")?.checked || false
        },
        useBottomColor: document.getElementById("useBottomColor")?.checked || false
      };
...
function bindWordModeEvents() {
  ...
  document.addEventListener('click', (e) => {
    if (e.target.classList.contains('wm-item')) {
      // アイテム選択
